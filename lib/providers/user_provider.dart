...
class UserProvider extends ChangeNotifier {
  ...
  Future<void> saveSettings(UserSettings newSettings) async {
    try {
      ...
      // uid를 포함한 설정 생성
      final settingsWithUid = newSettings.uid != userId
          ? newSettings.copyWith(uid: userId)
          : newSettings;

      final dataToSave = settingsWithUid.toMap();
  ...
  /// 선택된 목표 아이템 업데이트
  Future<void> updateSelectedGoal(String goalId) async {
    try {
      final userId = _documentId;
      if (userId == null) {
        throw Exception('로그인이 필요합니다');
      ...
      debugPrint('목표 선택 업데이트 중 오류 발생: $e');
      rethrow;
    }
  }
  ...
  /// 닉네임 중복 확인
  /// 현재 사용자의 닉네임은 중복으로 처리하지 않음
  Future<bool> checkNicknameDuplicate(String nickname) async {
    try {
      final currentUserId = _documentId;
      if (currentUserId == null) {
        throw Exception('로그인이 필요합니다');
      }

      // 현재 사용자의 닉네임과 같으면 중복 아님
      if (_settings.nickname == nickname.trim()) {
        return false;
      }
  ...
  /// 계정 삭제 (금연 포기)
  /// Firestore 문서와 Firebase Auth 계정을 모두 삭제
