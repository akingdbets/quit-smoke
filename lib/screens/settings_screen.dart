...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      if (date.year != _selectedYear || date.month != _selectedMonth || date.day != _selectedDay) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (date.isBefore(DateTime(1900, 1, 1))) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('1900년 이후 날짜를 선택해주세요')),
          );
        }
        return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('올바른 날짜를 선택해주세요')),
        );
      }
      return;
    }
    ...
    try {
      ...
      final dateString = '${_selectedYear.toString().padLeft(4, '0')}-${_selectedMonth.toString().padLeft(2, '0')}-${_selectedDay.toString().padLeft(2, '0')}';
      ...
      // 저장 성공 시
      if (mounted) {
        // Navigator 스택에 있다면 (AppSettingsScreen에서 열린 경우) pop
    ...
    return Container(
      ...
      child: DropdownButtonFormField<int>(
        ...
        onChanged: (value) {
          setState(() {
            _selectedYear = value;
            // 월이 선택되어 있고, 선택한 일이 새 월의 마지막 일보다 크면 일을 조정
      ...
      child: DropdownButtonFormField<int>(
        ...
        onChanged: (value) {
          setState(() {
            ...
            // 일이 선택되어 있고, 선택한 일이 새 월의 마지막 일보다 크면 일을 조정
